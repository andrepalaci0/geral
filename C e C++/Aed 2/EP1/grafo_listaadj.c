...
bool edgeValidation(int v, Graph* graph)
{
    if (v > graph->nNodes)
    {
        fprintf(stderr, "Erro: Num de vertices (%d) maior que o numero total de vertices (%d)\n", v, graph->nEdges);
        return false;
    }
    if(v < 0)
    {
...
{
    if(!edgeValidation(v, graph)) return false;
...
{
    if(!(edgeValidation(node1, graph)&& edgeValidation(node2, graph))) return false;

    Pointer q = graph->list[node1];
    while((q != NULL) && (q->vdest != node2)) q = q->prox;
...
{
    if(!(edgeValidation(node1, graph) && edgeValidation(node2, graph))) return NULLEDGE;
    Pointer q = graph->list[node1];
    while((q != NULL) && (q->vdest != node2)) q->prox;
...
{
    if(!(edgeValidation(node1, graph) && edgeValidation(node2, graph))) return;

    Pointer q;
    if(!(q = (Pointer) calloc(1, sizeof(Edge))))
    {
        fprintf(stderr, "Erro: falha na alocacao de memoria na funcao edgeInsert\n");
        return;
    }
    q->vdest = node2;
    q->peso = peso;
...
void maxTree(int nNodes, int node1, int node2, Graph* graph, FILE* saida)
{
    ...
    fflush(saida); //se nao ouver o flush de buffer, acontece erro de ponteiro
